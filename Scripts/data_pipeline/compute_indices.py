...
def compute_fci(df: pd.DataFrame, lci: pd.Series) -> pd.Series:
    ...
    # History-preserving: LCI (0.10) is plumbing-gated to 2018, but the other 90% of weight
    # (HY OAS, NFCI, curve, VIX) spans back to 1996. weighted_sum_coverage imputes the missing
    # LCI to neutral pre-2018 so FCI extends to its real component history instead of truncating
    # to the SOFR era. (2026-07-18 backfill: was nan_weighted_sum/strict -> gated FCI to 2018.)
    fci = weighted_sum_coverage([
        (z_hy, 0.25),
        (z_nfci, 0.25),
        (z_curve, 0.25),
        (z_vix, 0.15),
