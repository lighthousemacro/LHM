#!/usr/bin/env python3
"""
Print-authored PDF for the "Two Economies" Beacon.

Single source of truth: the published HTML (beacon_2026_04_19_PUBLISH_READY
.html). The HTML has text and the 18 charts already interleaved in document
order with correct figcaptions. We re-flow that body through reportlab so
pagination is controlled (no sliced charts, no orphan-whitespace pages),
matching the Horizon / Labor pristine bar.

The companion beacon_body.md is NOT used: its editorial figure numbering is
out of sync with the published HTML (md "Figure 3" == HTML "Figure 4").

Output:
    /Users/bob/LHM/Deliverables/Marc_Mishkin_2026-05-15/
        Lighthouse Macro — Two Economies (Beacon).pdf
"""
...
SRC = Path("/Users/bob/LHM/Outputs/Content/beacon_2026_04_19_two_economies")
HTML_PATH = SRC / "beacon_2026_04_19_PUBLISH_READY.html"
OUT_PATH = Path("/Users/bob/LHM/Deliverables/Marc_Mishkin_2026-05-15/"
                "Lighthouse Macro — Two Economies (Beacon).pdf")
...
def image_block(fig: Tag):
    ...
    if figcap:
        # Some figcaptions hold both the numbered caption and a source span.
        srcspan = figcap.find(class_="lhm-figsrc")
        src_txt = srcspan.get_text(" ", strip=True) if srcspan else ""
        if srcspan:
            srcspan.extract()
        cap_txt = figcap.get_text(" ", strip=True)
        if cap_txt:
...
SKIP = {"lhm-footer", "lhm-toc", "lhm-header-rule"}
...
def build():
    ...
    root = soup.find(class_="lhm-beacon") or soup.body
    ...
    doc = SimpleDocTemplate(
        ...
        title="Two Economies — The Beacon", author="Lighthouse Macro",
    )
    doc.build(story, onFirstPage=_page, onLaterPages=_page)
    print(f"figure blocks: {fig_count}")
