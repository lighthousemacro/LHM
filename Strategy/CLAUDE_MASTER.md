...
Each color has a narrative role, not just a slot in a rotation. Ocean is the protagonist (the Lighthouse). Dusk is the antagonist (the contrast). Sky is the sidekick (the main homie, the one that shares Ocean's burden). Dusk and Sky are equally important behind Ocean — they just play different roles.

| Name | Hex | Mnemonic | Narrative Role | Usage |
|------|-----|----------|----------------|-------|
| **Ocean** | `#2389BB` | 23+89+BB | **Protagonist** | Primary brand, dominates every visual, headers, borders, chart primary |
| **Dusk** | `#FF6723` | 23 | **Antagonist** | Pairs against Ocean in 2-series overlays; signal vs target; accent bar |
| **Sky** | `#23BBFF` | 23+BB | **Sidekick** | 3rd line in charts. Supporting visual elements. Shares Ocean's burden. Equally important as Dusk. |
| **Sea** | `#00BB89` | BB+89 | Situational — win | 4th line, on-target regime bands |
| **Venus** | `#FF2389` | 23+89 | Situational — alert | 4th line, 2% target lines, critical alerts |
| **Doldrums** | `#898989` | 89+89 | Neutral | Axis spines, labels, secondary text |
| **Starboard** | `#238923` | 23+89 | Regime — bullish | Bullish regime semantics only |
| **Port** | `#892323` | 89+23 | Regime — bearish | Bearish regime / crisis bands only |

### Color Hierarchy (How To Apply)

**Charts:**
| Series count | Color order |
|---|---|
| 1 | Ocean |
| 2 (signal vs target overlay) | Ocean + Dusk |
| 3 | Ocean + Dusk + Sky |
| 4 | Ocean + Dusk + Sky + (Sea or Venus depending on semantic) |
| 5 | Ocean + Dusk + Sky + Sea + Venus |

Sky against Sea is low-contrast — if Sky is primary (rare), swap Venus into the 4-slot ahead of Sea.

**Branded HTML / reports:**
- Header bands, dominant numbers, primary CTAs → Ocean
- Antagonist callouts (corrections, "what consensus thinks") → Dusk
- Supporting callouts, sidebar elements, secondary highlights → Sky. *Sky must appear; do not skip from Ocean+Dusk to Sea/Venus.*
- Win callouts → Sea
- Fail / alert callouts → Venus
- Starboard / Port → ONLY for bull/bear regime semantics, never for neutral comparisons
