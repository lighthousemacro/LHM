...
**Still open:**
- **Crypto fetcher repair** — SLI/CTI/CVI/CDI/CFI/CRYPTO_* feed-down since 2026-02-02. Diagnosed: free price data flows to `observations` (fresh) but the index layer reads the retired `crypto_scores`/`crypto_metrics` tables; fundamentals (P/E, active-devs) are paid-feed-blocked. Plan (Bob: rewire on free data, retire fundamentals): rewrite `compute_crypto_indices` onto DefiLlama `DEFI_*_TVL`/`STABLE_*`/`CRYPTO_*_PRICE` from `observations` + DefiLlama historical backfill; retire the fundamental indices. Lowest priority (revenue-first).
