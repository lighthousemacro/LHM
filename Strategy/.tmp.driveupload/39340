...
| **ECI: Private Industry** | CIU2010000000000I | Quarterly | Level lags 1-2 qtrs | Excludes public sector |
...
| **Not in Labor Force: Want Job** | NILFWJN | Monthly | Level lags | Shadow labor supply |
| **Discouraged Workers** | LNU05026645 | Monthly | Level lags | Gave up searching, subset of NILFWJN |
...
|---|---|---|---|---|---|
| **Headline Employment** | BLS (CES) | Monthly | ~5 days | FRED + BLS API | Yes |
| **Unemployment by Demographic** | BLS (CPS) | Monthly | ~5 days | FRED + BLS API | Yes |
| **JOLTS (Quits, Hires, Openings)** | BLS | Monthly | ~40 days | FRED + BLS API | Yes |
| **Jobless Claims** | DOL | Weekly | ~3 days | FRED | Yes |
| **Wage Growth by Segment** | Atlanta Fed | Monthly | ~15 days | FRED | Yes |
| **ECI / Unit Labor Costs** | BLS | Quarterly | ~30-45 days | FRED | Yes |
| **Productivity** | BLS | Quarterly | ~45 days | FRED | Yes |
| **State Unemployment** | BLS (LAUS) | Monthly | ~3 weeks | FRED | Partial |
| **Firm Size Dynamics** | BLS (BED) | Quarterly | ~7 months | BLS Website (free) | No |
| **Real-Time Postings** | Indeed Hiring Lab | Daily | ~1 day | FRED (IHLIDXUS) | Yes (after pipeline run) |
| **Layoff Announcements** | Challenger | Monthly | ~1 week | Press Release (free) | No |
| **County-Level Employment** | BLS (QCEW) | Quarterly | ~6 months | BLS API (free) | No |
| **Sahm Rule** | FRED | Monthly | Same day | FRED | Yes (after pipeline run) |
| **ADP Employment** | ADP/Moody's | Monthly | ~2 days before BLS | FRED | Yes |
| **Sector Employment** | BLS (CES) | Monthly | ~5 days | BLS API | Yes |
| **Compensation (BEA)** | BEA (NIPA T20100) | Quarterly | ~30 days | BEA API | Yes |
| **Chicago Fed LMCI** | Chicago Fed | Monthly | ~3 weeks | chicagofed.org (free) | No |
| **Philadelphia Fed State Indexes** | Philadelphia Fed | Monthly | ~3 weeks | philadelphiafed.org (free) | No |

**Data Pipeline Sources:**
- **FRED API:** Primary source for ~90 labor series (free, 11893c key)
- **BLS API:** Direct source for CES/CPS/JOLTS (free, 2e66a key), provides sector detail not on FRED
- **BEA API:** Compensation components via NIPA tables (free, 4401D key)
- **Atlanta Fed:** Wage Growth Tracker now available on FRED (6 series)
- **Indeed:** Job Postings Index now available on FRED (IHLIDXUS)
- **TradingView:** Available for visual cross-reference of labor indicators (Bob's subscription)
...
**"Labor: The Source Code"** (Educational Series, January 2026) is the published article version of this pillar, covering the flows vs. stocks framework, the transmission chain (Labor → Income → Spending → Credit → Housing), and the key insight that "when workers stop quitting, they're signaling diminished options before layoffs materialize." The article introduces the LPI (Labor Pulse Index) and LFI (Labor Fragility Index) as the master composites for regime classification.
