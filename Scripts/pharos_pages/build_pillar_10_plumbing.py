...
def build():
    ...
    # ON RRP ran occasional test operations before the facility went daily in
    # Sep 2013; start at the dense era so the line never bridges phantom years
    rrp = load_obs("RRPONTSYD")
    rrp_b64, _ = chart_lines(
        [(rrp, "RRP balance, $B")],
        thresholds=[(200, "$200B", DUSK, "--", 1.0)],
        fmt="{:,.0f}", legend_loc="upper left",
        start=pd.Timestamp("2013-09-23"),
    )
    effr = load_obs("EFFR")
    iorb = load_obs("IORB")
